...
网站源代码使用符号连接处理“在不同位置有多个内容相同的文件”这种情况。如果网站运行在Windows中，建议在WSL内`git clone`或者解压`zip`，以保留符号连接。如果网站运行在Linux中，请不要在Windows中解压源代码然后重新打包，否则符号连接会丢失，你可以考虑直接在Linux里通过命令行下载源代码并解压，或者使用`git clone`命令获取源代码。此外，在`Git for Windows`中启用符号连接支持也无法解决该问题，参见步骤9中的描述。
