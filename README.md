...
此外，在`Git for Windows`中启用符号连接支持也无法解决该问题，参见步骤9中的描述。
...
4. `tpl/classic/img` 文件夹里面是图片，比如 `hulvlin2.gif` 是默认logo。里面的`face`文件夹放的是表情包。表情包的文件名命名规则详见 [这里](src/tpl/classic/img/face/README.md)。
