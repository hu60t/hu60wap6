...
{else}#旅行者#{div class="righttext buttonbox"}{span class="button"}<a href="user.login.{$bid}?u={urlencode($page->geturl())}">登陆</a>{/span}{span class="button"}<a href="user.reg.{$bid}?u={urlencode($page->geturl())}">注册</a>{/span}{/div}{/if}
</div>{/if}
{/if}