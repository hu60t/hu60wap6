...
<div class="content">
    {foreach $list as $k}
        ...
        <div class="i">
        <div class="floor_content user-content" id="floor_content_{$k.lid}">{if $onlyReview}<a class="floor-link" href="addin.chat.{urlencode($k.room)}.{$bid}?floor={$k.lid}#{$k.lid}">{$k.room} {$k.lid}楼</a>{else}<a class="floor-link" name="{$k.lid}" href="?{$k.lid}#{$k.lid}">{$k.lid}</a>{/if}. {$ubbs->display($k.content,true)}</div>
