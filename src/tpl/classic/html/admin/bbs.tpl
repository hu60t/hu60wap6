...
<form action="admin.bbs.bk.{$bid}" method="post">
  ...
  <p>
    {select name="parent_id" option=$array}<br/>
    {if $xg['parent_id']!=0}版主列表：
    <input type="text" name="bz" value="{$xg['bz']}">(用ID表示，以“,”隔开)<br/>{/if}
  ...
  <form action="admin.bbs.bk.{$bid}" method="post">
    <div class='cr180_form'>
      <div >
        ...
        <p>
          <input type="submit" name="xg" id="submit" class="cr_login_submit" style="width:49%;" value="修改论坛板块" />
          <input type="submit" name="sc" id="submit" class="cr_login_submit" style="width:49%;" value="删除论坛板块" />
        </p>
        </div>
      </div>
    </form>
