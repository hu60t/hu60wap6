...
4. 打开 https://xinghuo.xfyun.cn/ 并登录。
...
document.run = async function() {
    while (true) {
        try {
            ...
            // {"data": [
            //    {"uid":3, "text":"@讯飞星火，你好"},
            //    {"uid":2, "text":"@讯飞星火，我有一个问题"},
            //    {"uid":1, "text":"@讯飞星火，刷新页面"},
            // ]}
            let exceptionCount = 0;
            for (let i=0; i<messages.data.length; i++) {
                // 要发给讯飞星火的话，开头包含的“@机器人名称，”会被后续流程自动去除。
...
// 模型对应关系
const modelMap = {
    1 : 0, // 讯飞星火没有模型切换功能，保留接口备用
};

/////////////////////////////////////////////////////////////

// 聊天框的CSS选择器
const chatBoxSelector = 'div[class^="ask-window_ask_window_wrapper__"] textarea';
...
const chatLineSelector = 'div[class^="chat-window_chat_content__"] div[class^="chat-window_content_user__"], div[class^="chat-window_chat_content__"] div.markdown-body';

// 聊天回答的CSS选择器
const chatReplySelector = 'div[class^="chat-window_chat_content__"] div.markdown-body';

// 左侧会话列表项的CSS选择器
const sessionListItemSelector = 'div[class^="pages_list_item__"]';

// 当前会话的CSS选择器
const currentSessionSelector = 'div[class*=" pages_list_item_active__"]';

// 编辑、删除、确认、取消按钮的CSS选择器
const actionButtonSelector = 'div[class^="pages_control_btn__"] div[class^="pages_img_wrap__"]';

// 删除确认按钮的CSS选择器
const deleteConfirmSelector = 'div.ant-modal-confirm-btns button.ant-btn';
...
async function renameWant() {
    if (wantRename !== null) {
        // 距离回复不到2秒，等够2秒
        // 防止重命名过程中讯飞星火同时自动重命名，导致我们的名称保存失败
...
async function sendRequest(text, uid) {
    ...
    // 分割指令
    //  @讯飞星火[ 模型序号][ 代码格式[=参数]]，发言内容
    // 示例：
    //  @讯飞星火，你好
    //  @讯飞星火 2，你好
    //  @讯飞星火 html，输出一段html hello world
    //  @讯飞星火 2 html，输出一段html hello world
    //  @讯飞星火 html=500，输出一段html hello world
    //  @讯飞星火 2 html=300x500，输出一段html hello world
        ...
        if (turndownService && typeof turndownPluginGfm == 'object') {
            ...
            turndownService.addRule('highlightedCodeBlock', {
                ...
                replacement: function (content, node, options) {
                    ...
                    var lang = node.querySelector('span.code-block-header__lang')?.textContent || ''; // lang span可能不存在
                    var code = node.querySelector('div.inner-code').textContent.replace('/^\n+|\n+$/g', '');
                    var fence = (() => {
...
async function runOnce() {
    ...
    try {
        if (document.querySelector(chatHistorySelector)) {
            document.querySelector(chatHistorySelector).click();
        }
